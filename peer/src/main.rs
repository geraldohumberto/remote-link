...
impl App {
    ...
    fn do_connect(&mut self) {
        ...
        self.config.last_host = host.clone(); self.config.last_port = port; self.config.save();
...
impl App {
            ...
            ui.vertical_centered(|ui| {
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label(RichText::new("A senha sera usada para autenticar conexoes neste PC.").size(11.0).color(Color32::DARK_GRAY));
                        ui.add_space(16.0);
                        ...
                        if ui.add(btn).clicked() {
                            if self.fr_pass1.is_empty() {
                                self.fr_error = "A senha nao pode ser vazia.".into();
                            } else if self.fr_pass1 != self.fr_pass2 {
                                self.fr_error = "As senhas nao coincidem.".into();
                            } else {
    ...
    fn ui_main(&mut self, ctx: &egui::Context) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.add_space(20.0);
            egui::Frame::none().fill(Color32::from_rgb(19, 19, 26)).rounding(10.0).inner_margin(egui::Margin::same(16.0)).show(ui, |ui| {
                ui.horizontal(|ui| {
                ...
                ui.add_space(4.0);
                ui.label(RichText::new("Passe este IP + senha para quem vai se conectar aqui.").size(11.0).color(Color32::DARK_GRAY));
            });
            ui.add_space(20.0);
            ui.separator();
            ui.add_space(16.0);
            ...
            egui::Frame::none().fill(Color32::from_rgb(19, 19, 26)).rounding(10.0).inner_margin(egui::Margin::same(16.0)).show(ui, |ui| {
                egui::Grid::new("conn_grid").num_columns(2).spacing([8.0, 10.0]).show(ui, |ui| {
                    ...
                    ui.add(egui::TextEdit::singleline(&mut self.host_buf).desired_width(f32::INFINITY).hint_text("192.168.1.100"));
                    ui.end_row();
                    ui.label(RichText::new("Porta").size(12.0).color(Color32::GRAY));
                    ui.add(egui::TextEdit::singleline(&mut self.port_buf).desired_width(80.0).hint_text("7890"));
                    ui.end_row();
    ...
    fn ui_remote(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("remote_bar")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.label(RichText::new(format!("{} {}x{}", self.peer_platform, self.server_w, self.server_h)).size(12.0).color(Color32::from_rgb(0, 212, 255)));
                    ui.separator();
                    ...
                    if ui.small_button("Clipboard").clicked() {
                        if let Ok(mut c) = arboard::Clipboard::new() {
                            if let Ok(text) = c.get_text() { self.send_cmd(Cmd::Clipboard(text)); }
                        }
                    }
            ...
            .show(ctx, |ui| {
                ...
                if let Some(tex) = &self.frame_tex {
                    ...
                    let to_srv  = |p: egui::Pos2| -> (i32, i32) {
                        (((p.x - rect.left()) * scale_x) as i32, ((p.y - rect.top()) * scale_y) as i32)
                    };
    ...
    fn ui_files(&mut self, ctx: &egui::Context) {
        ...
        egui::SidePanel::right("upload_panel").resizable(false).default_width(200.0)
            ...
            .show(ctx, |ui| {
                ...
                if ui.button("Enviar arquivo...").clicked() {
                    // file picker nao incluido — adicione rfd ao Cargo.toml para habilitar
                }
                ui.add_space(12.0);
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                for (i, item) in self.file_items.iter().enumerate() {
                    ...
                    let label    = format!("{} {}  ({})", icon, item.name, size_str);
    ...
    fn ui_settings(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("cfg_bar")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Configuracoes").size(14.0));
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let btn = egui::Button::new(RichText::new("Salvar").size(14.0))
                .fill(Color32::from_rgb(0, 180, 220)).min_size(Vec2::new(160.0, 34.0));
            if ui.add(btn).clicked() {
            ...
            if self.cfg_saved {
                ui.add_space(8.0);
                ui.label(RichText::new("Salvo! Reinicie o app para aplicar mudanca de porta.").size(12.0).color(Color32::from_rgb(34, 211, 165)));
            }
        });
    }
}
