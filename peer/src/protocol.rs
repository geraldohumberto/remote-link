// src/protocol.rs — Protocolo de comunicação RemoteLink
// Wire format: [4 bytes BE len][JSON payload] ou [4 bytes BE len][JSON header][bytes raw]
...
pub enum Message {
    ...
    // Tela: FrameInfo precede bytes raw do JPEG
    ...
    FileChunk    { size: u32 },   // precede bytes raw
